...
_COUNTER_TITLES = {
    ...
    repo.COUNTER_NOT_SELECTED: "✂️ Не выбрано (отбор)",
...
async def build_stats_text(
    ...
    lines.append(f"📥 Ожидают дедупа: {processing_queued}")
    lines.append(f"⚙️ В дедупе: {processing_active}")
    lines.append(f"⏳ Ждут отбора: {selection_queued}")
