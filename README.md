...
- Дедупликация: векторный поиск ближайших новостей (pgvector) + LLM-вердикт; тот же вызов решает,
  стоит ли вообще писать пост (гейт важности).
- Дневной лимит постов (`limits.daily_posts`) с равномерным распределением: исчерпан — автоскип;
  LLM держит резерв на важные новости и пропускает малозначимые (`skipped`).
...
- **Жёсткий лимит** (код): если лимит исчерпан, новые новости автоматически получают статус `skipped`
  (окончательно, без LLM и эмбеддинга) до конца дня. При `daily_posts: 0` лимит отключён и счётчик
  не запрашивается вовсе.
- **Гейт важности** (LLM, часть дедуп-вердикта): в промпт дедупа передаётся контекст публикации —
  текущее время, сколько постов вышло сегодня, лимит и резерв `limits.reserve_posts`. Модель решает
  не только «дубликат или нет», но и «стоит ли вообще писать пост»: уникальная, но малозначимая
  новость отправляется в `skipped`. В промпте подчёркнута равномерность публикаций в течение дня
  и необходимость держать резерв (~`reserve_posts` постов) для действительно важных новостей.

Скипнутые новости (`skipped`) — терминальный статус: посты не создаются, завтра не переобрабатываются.
Счётчик виден в `/admin` («✂️ Скипнуто»).
...
RSS feeds → Poller → [rss.clear_run: при старте всё, что уже в лентах, → статус cleared, конец]
  ...
  → [1] Лимит: если limits.daily_posts > 0 и лимит на сегодня исчерпан → skipped, конец (без LLM)
  → [2] Дедупликация + гейт важности (один LLM-вызов): pgvector top-5 за window_days
        с порогом min_similarity + вердикт (дубликат / публиковать / пропустить);
        в промпт передаётся контекст публикации (время, счётчик постов, лимит, резерв)
      → duplicate / needs_review (on_error: review|pass|drop) / skipped (неважная) — конец
  ...
  → [4] Поиск релевантных опубликованных постов (top-3, context.window_days);
  ...
  → [6] Публикация: auto (сразу в очередь; append_source — ссылка «Источник» в конце поста;
...
`pending → dedup → (duplicate | needs_review | skipped | photo_search) → writing → (moderation | published)`,
`rejected` — отклонение на модерации или таймаут, `failed` — при ошибках этапа или коротком тексте,
`skipped` — новость пропущена: исчерпан дневной лимит (`limits.daily_posts`) или LLM-гейт важности
решил, что пост не стоит (уникальна, но малозначима); терминальный статус, посты не создаются,
...
- `/admin` — статистика канала (считается SQL-запросами
