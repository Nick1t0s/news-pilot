...
прогоняет новость через единый LLM-гейт (дедупликация + важность по эмбеддингам), подбирает фото
через ИИ-агента с инструментами, генерирует пост в стиле канала (с ответом на прошлый пост при
развитии истории) и публикует его в Telegram.

HTTP-сервера в приложении нет: источник — RSS, мониторинг — логи и админ-меню бота.

## Архитектура в двух словах

- **Пайплайн целиком в памяти**: очередь новостей (`FeedItem`) и очередь постов (`PublishJob`) —
...
- **БД — 2 таблицы**: `posts` (только опубликованные посты + эмбеддинги, pgvector) и `counters`
  (накопительные счётчики: дубликаты, скипы, ошибки, clear-run). Схема создаётся автоматически.
- **Поток**: poller кладёт `FeedItem` в очередь → X независимых воркеров (семафор `pipeline.concurrency`)
  гонят каждый свой эмбеддинг → pgvector-поиск по опубликованным постам → один LLM-вызов решает
  «дубликат/не дубликат» и «стоит ли постить» → не постим — инкремент счётчика; постим — фото-агент,
  поиск контекста, генерация, и `PublishJob` уходит в очередь постинга → один асинхронный отправитель
  публикует в Telegram и записывает пост с эмбеддингом в `posts`.
- **Ограничения**: дедуп — только против опубликованных постов (дубликат пропущенной новости пройдёт
  цикл заново, но его отсеет гейт важности); черновики модерации и незавершённая работа живут в памяти
...
- Дедупликация: векторный поиск ближайших опубликованных постов (pgvector) + LLM-вердикт; тот же вызов
  решает, стоит ли вообще писать пост (гейт важности).
- Дневной лимит постов (`limits.daily_posts`) с равномерным распределением: исчерпан — автоскип;
  LLM держит резерв на важные новости и пропускает малозначимые (счётчик скипов).
...
- Очереди не персистентны: при рестарте незавершённая работа (новости в очереди обработки, посты в
  очереди публикации, черновики модерации) теряется — это осознанное упрощение, состояние пайплайна
  живёт в памяти; в БД хранятся только опубликованные посты и счётчики.
...
| `limits` | `daily_posts` | `0` | дневной лимит постов; `0` = без жёсткого лимита (счётчик не запрашивается) |
| | `reserve_posts` | `5` | резерв постов на важные новости (подсказка для LLM-гейта) |
| | `timezone` | `Europe/Moscow` | часовой пояс, по которому считается «сегодня» |
...
### Дневной лимит и гейт важности

`limits.daily_posts` — максимум постов за сутки (считаются опубликованные посты с начала «сегодня»
в часовом поясе `limits.timezone`). Механизмы:

- **Жёсткий лимит** (код): если лимит исчерпан, новые новости автоматически скипаются
  (окончательно, без LLM и эмбеддинга) до конца дня — счётчик `limit_skipped`.
  При `daily_posts: 0` лимит отключён и счётчик не запрашивается вовсе.
- **Гейт важности** (LLM, часть дедуп-вердикта): в промпт дедупа передаётся контекст публикации —
  текущее время, когда вышел последний пост (сколько минут/часов назад), сколько постов вышло сегодня,
  лимит и резерв `limits.reserve_posts`. Модель решает не только «дубликат или нет», но и «стоит ли
  вообще писать пост»: уникальная, но малозначимая новость скипается (счётчик `skipped_unimportant`).
  Промпт инструктирует балансировать частоту по свежести последнего поста: пост только что —
  пропускать мелочь, постов давно не было — публиковать умеренно значимые. Подчёркнуты равномерность
  публикаций в течение дня и резерв (~`reserve_posts` постов) для действительно важных новостей.

Скипнутые новости нигде не запоминаются (нет таблицы статусов): посты не создаются, счётчики растут.
...
RSS feeds → Poller → [rss.clear_run: при старте всё, что уже в лентах, → счётчик cleared, конец]
  ...
  → FeedItem в очередь (толстый объект, всё в атрибутах)
  → X независимых воркеров (семафор pipeline.concurrency):
      [1] Лимит: если limits.daily_posts > 0 и лимит на сегодня исчерпан → счётчик limit_skipped, конец
      [2] Гейт: эмбеддинг (Ollama, вычисляется один раз) → pgvector top-K опубликованных постов
          за window_days с порогом min_similarity → один LLM-вызов: дубликат? стоит ли постить?
          (в промпт передаётся контекст публикации: время, счётчик постов, лимит, резерв)
        → duplicate → счётчик duplicates, конец
        → малозначимая → счётчик skipped_unimportant, конец
      [3] Фото-агент: tavily_image_search / inspect_image (vision) / select_images,
          лимиты max_iterations/max_searches/max_images; 0 фото — валидный исход; сбой не блокирует пост
      [4] Поиск релевантных опубликованных постов (top-3, context.window_days) по тому же эмбеддингу;
          продолжение истории публикуется ответом на прошлый пост, без оборотов «как мы писали ранее»
      [5] Генерация поста по prompts/style.md (structured output, ≤1000 символов, HTML;
...
Сбой этапа не роняет пайплайн: обработка сразу повторяется с начала (`pipeline.retries` попыток);
после исчерпания — счётчик `failed`. Промежуточные статусы не пишутся: исход новости — либо пост в
канале (строка в `posts`), либо инкремент счётчика. Сбой фото/Tavily не блокирует публикацию.
...
| `skipped_unimportant` | уникальна, но гейт важности решил не постить |
| `limit_skipped` | дневной лимит исчерпан (без LLM) |
| `failed` | ошибка этапа после ретраев / слишком короткий текст / сбой публикации |
...
Компромисс: дедуп смотрит только на опубликованные посты. Пропущенная новость забывается, и её
дубликат из другой ленты пройдёт цикл заново — но его, скорее всего, снова пропустит гейт важности.
...
- `/admin` — статистика канала: опубликованные посты (24ч/7д/всего) из `posts`,
  накопительные счётчики из `counters`, очередь и обработка в памяти, остаток дневного
  лимита, черновики и очередь публикации, источники за 24ч.
...
- интеграция: полный пайплайн на моках внешних сервисов (LLM/Tavily/Telegram — фейки,
  БД — реальный Postgres): гейт дедупа/важности, счётчики, clear-запуск poller, модерация
  в памяти (включая таймаут), reply-цепочки, добавление источника к посту.
...
app/
  ...
  pipeline/processor.py воркеры над FeedItem: гейт дедупа/важности → фото → генерация
                        → PublishJob в очередь постинга; исходы — счётчики
  dedup.py              векторный поиск по published-постам + LLM-вердикт (дедуп + важность)
