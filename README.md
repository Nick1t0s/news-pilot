...
- Генерация поста в стиле канала (HTML, ≤1000 символов) — стиль и контентные правила задаются в `prompts/style.md`:
...
| `pipeline` | `retries` | `2` | ретраи всей обработки: при ошибке сразу повторная попытка, после исчерпания — `failed` |
...
по одной за раз. Сбой этапа не роняет очередь: обработка сразу повторяется с начала
(`pipeline.retries` попыток, warn-записи в processing_log); после исчерпания попыток или
если строки новости уже нет — `status=failed`. Если черновик уже ушёл в публикацию/
модерацию (`queued`/`moderation`/`published`), ретрай не запускается — за исход отвечает
publish-путь. Осиротевшие черновики (`draft`/`failed`) от неудавшейся попытки удаляются
перед повтором.
